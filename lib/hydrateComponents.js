...
function hydrateComponents(components, vueOptions = {}) {
  ...
  return components.map(component => {
    ...
    const instances = [...elements].map(el =>
      new Vue({ ...vueOptions, render }).$mount(el)
    );
...
function showWarningsForUnprovidedComponents(components) {
  ...
  [...elements].forEach(el => {
    ...
    const componentProvidedForEl = components.find(
      ({ name }) => name === attrName
    );
    if (!componentProvidedForEl)
      console.warn(
        `Vue Moisturizer: You did not provide the component "${attrName}" in the client`
      );
  });
}
...
function throwErrorforDuplicateNames(components) {
  ...
  if (duplicates.length) {
    const duplicateComponentNames = duplicates
      .map(comps => `"${comps[0].name}"`)
      .join(", ");
    const infoMsg = [
      "You are trying to hydrate multiple components that have the same name!",
      `Please provide different names for components using ${duplicateComponentNames}`
    ];
...
function getDuplicates(components) {
  return Object.values(groupComponents(components)).filter(
