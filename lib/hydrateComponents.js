...
function hydrateComponents(components, vueOptions = {}) {
	...
	const state = typeof window !== 'undefined' ? window.__MOISTURIZER_STATE__ : {};

	return components.map(component => {
		const elements = document.querySelectorAll(`[${ATTR}="${component.name}"]`);
		const props = state[component.name] ? state[component.name].props : {};
		const instances = [...elements].map(el =>
			new Vue({
				...vueOptions,
				render(h) {
					const attrs = el.hasAttributes()
						? convertAttributesToHashMap(el.attributes)
						: {};
