...
class Slots {
	...
	serialize(slots) {
		if (!slots.default) return this.stringify({'default':[{'__moisturizer':false,'type':'text','text':'no slots found'}]})
		const convertedDefaultSlot = slots.default.map(slot => this.serializeSlot(slot));
